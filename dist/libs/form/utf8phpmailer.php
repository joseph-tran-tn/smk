...
//PHPMailerを読み込む
// FOR PHP 5.6
// require(APP_PATH."libs/form/phpmailer/PHPMailerAutoload.php");

// FOR PHP 7
require_once(APP_PATH."libs/form/phpmailer/class.smtp.php");
require_once(APP_PATH."libs/form/phpmailer/class.phpmailer.php");
...
class JPHPMailer extends PHPMailer {
	// var $CharSet = "iso-2022-jp"; // cmt out 170104
	var $CharSet = "UTF-8";
	var $Encoding = "7bit";
...
/*	function addAttachment($address,$name="") {
	...
	function setSubject($subject){
		$this->Subject = $this->encodeMimeHeader(mb_convert_encoding($subject,"UTF-8",$this->in_enc)); // cmt out 170104
	...
	*/
	...
	function setFromName($fromname){
		$this->FromName = $this->encodeMimeHeader(mb_convert_encoding($fromname,"UTF-8",$this->in_enc)); // cmt out 170104
	...
	function setBody($body){
		$this->Body = mb_convert_encoding($body,"UTF-8",$this->in_enc); // cmt out 170104
		// $this->Body = $body;
		$this->AltBody = "";
		$this->IsHtml(false);
	}
	...
	function setHtmlBody($htmlbody){
		$this->Body = mb_convert_encoding($htmlbody,"UTF-8",$this->in_enc); // cmt out 170104
		// $this->Body = $htmlbody;
		$this->IsHtml(true);
	}
	...
	function setAltBody($altbody){
		$this->AltBody = mb_convert_encoding($altbody,"UTF-8",$this->in_enc); // cmt out 170104
